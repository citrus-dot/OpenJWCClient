# chat-daily-me Specification

## Purpose
TBD - created by archiving change ios-chat-daily-me. Update Purpose after archive.
...
消息流 SHALL 按「轮」展示：每轮为工具活动区（若有）在上 + 消息气泡在下。发送消息 SHALL：插入用户消息 → 插入空 assistant 占位（RUNNING）→ 启动 Agent 循环，逐事件更新 UI 与落库（runId、工具卡逐条、流式正文、终态）。生成中的流式正文 SHALL 只在内存叠加展示（不写库、以纯文本渲染并以约 100ms 批量合并驱动，避免逐 token 重排）；消息完成后气泡 SHALL 切换为 Markdown 渲染。切会话 SHALL 自动重订阅该会话数据。滚动 SHALL 三态：生成中自动跟随、用户上滑即停止跟随并显示回底按钮、回底后恢复跟随。

#### Scenario: 发送并流式生成
- **WHEN** 用户发送消息且 LLM 配置有效
- **THEN** 用户气泡立即出现，助手侧显示生成中指示，正文随流式事件以打字机效果更新，完成后正文以 Markdown 渲染且已持久化
...
#### Scenario: 上滑停止跟随
- **WHEN** 生成中用户上滑离开底部
- **THEN** 列表停止自动滚动并出现回底按钮；点按或滑回底部后恢复自动跟随

### Requirement: 停止生成（iOS 交互增强，超出 Android 对齐基线）
流式生成期间发送键 SHALL 原位切换为停止键；点击停止 SHALL 取消当前 Agent 循环、该轮落库为 FAILED（agent_cancelled）并原位出现重试行（与失败重试同路径）。停止后已生成的部分正文 SHALL 保留展示。

#### Scenario: 停止生成
- **WHEN** 用户在流式生成中点击停止键
- **THEN** 生成停止，该轮以失败终态落库，已流出的部分正文保留，重试行出现

### Requirement: 工具卡片
每轮的工具调用 SHALL 收在可折叠的工具活动容器内：生成中自动展开并显示具体动作（如「正在读取资讯…」）；结束后收起为摘要行（如「已使用 N 个工具 · X 秒」），可手动展开查看逐卡详情（标题、summary 默认折叠、状态图标、耗时）。仅 `read_notice` 工具卡在带 targetId 时 SHALL 可点击跳转该资讯详情。

#### Scenario: 工具活动折叠
- **WHEN** 一轮含 3 个工具调用的消息生成完毕
- **THEN** 该轮工具区收起为「已使用 3 个工具」摘要行；点击展开可见逐卡状态与耗时
...
### Requirement: 失败与重试
Agent 循环失败（LLM 错误/中断/用户停止）SHALL：该轮 assistant 占位落库为 FAILED + 错误码（agent_failed / agent_interrupted / agent_cancelled），UI 显示错误态与「重试」行（固定位于最后一条用户消息之后）；重试 SHALL 复用原文本与附件（不重复插入用户消息）。错误码属配置相关（401/403/404/配置缺失类）时 SHALL 提供跳转 LLM 设置的入口。流未到终态的发送 SHALL 视为中断落库（agent_interrupted）。
...
会话列表、当前会话消息流（含工具卡组装）、日报完成稿列表 SHALL 由 GRDB ValueObservation 驱动（单闭包内完成多表读取与组装，任一相关表写入自动重发）；流式生成中的正文 SHALL 仅存在于内存，终态一次性落库后由观察刷新。App 重启后工具轨迹与消息 SHALL 从 DB 还原；观察 SHALL 在视图生命周期内启停，不泄漏。
